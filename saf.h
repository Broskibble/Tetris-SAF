...
uint8_t SDL_volume = 0;
int8_t SDL_currentSound = -1;
uint16_t SDL_soundPosition = 0;

#if SAF_SETTING_ENABLE_SOUND

void SAF_SDL_playSound(uint8_t sound)
{
  SDL_currentSound = sound;
  SDL_soundPosition = 0;
}
...
void SAF_SDL_audioFillCallback(void *userdata, uint8_t *s, int l)
{
  ...
  for (int i = 0; i < l; ++i)
  {
    if (SDL_currentSound < 0)
      *s16 = 0;
    else
    {
      *s16 = SAF_FE_getSoundSample(SDL_currentSound,SDL_soundPosition);

      SDL_soundPosition++;

      if (SDL_soundPosition >= SAF_FE_SOUND_SAMPLE_COUNT)
      {
        SDL_currentSound = -1;
        SDL_soundPosition = 0;
      }
...
const uint8_t *SDL_keyboardState;

SDL_Window *SDL_window;
SDL_Renderer *SDL_renderer;
SDL_Texture *SDL_texture;
uint8_t SDL_pixelArtUpscale = 0;
uint8_t *SDL_upscaleScreen = 0;
SDL_GameController *SDL_controller = 0;
...
void SAF_FE_GF_init(uint8_t CLIParameters[128])
{
  ...
  SDL_volume = CLIParameters['v'] - '0';

  if (CLIParameters['u'] != 0)
  {
    SDL_pixelArtUpscale = 1;
    SDL_upscaleScreen = malloc(SAF_SCREEN_WIDTH * SAF_SCREEN_WIDTH * 4);
  }
  ...
  SDL_window = SDL_CreateWindow(SAF_PROGRAM_NAME, SDL_WINDOWPOS_UNDEFINED,
  ...
  SDL_texture = SDL_CreateTexture(SDL_renderer,
    ...
    SAF_SCREEN_WIDTH * (SDL_pixelArtUpscale ? 2 : 1),
    SAF_SCREEN_HEIGHT * (SDL_pixelArtUpscale ? 2 : 1));
...
void SAF_FE_GF_end()
{
  ...
  if (SDL_pixelArtUpscale)
    free(SDL_upscaleScreen);
}

uint8_t SAF_FE_GF_loop(uint8_t params[128])
{
  SDL_volume = params['v'] - '0';
  ...
  SAF_FE_GF_present(const uint8_t screen[SAF_SCREEN_WIDTH * SAF_SCREEN_HEIGHT])
{
  if (SDL_pixelArtUpscale)
  {
    SAF_FE_scale2xScreen(screen,SDL_upscaleScreen);
    SDL_UpdateTexture(SDL_texture,NULL,SDL_upscaleScreen,SAF_SCREEN_WIDTH * 2);
  }
  else
    SDL_UpdateTexture(SDL_texture,NULL,screen,SAF_SCREEN_WIDTH);
...
uint8_t SAF_FE_GF_keyPressed(char keyChar)
{
  if (keyChar <= 'z' && keyChar >= 'a')
    return SDL_keyboardState[SDL_SCANCODE_A + (keyChar - 'a')];

  #define b(x) ((SDL_controller != NULL) && \
    SDL_GameControllerGetButton(SDL_controller,SDL_CONTROLLER_BUTTON_ ## x))

  switch (keyChar)
  {
    case 'U': return 
      SDL_keyboardState[SDL_SCANCODE_UP] || 
      b(DPAD_UP);
      break;

    case 'R': return 
      SDL_keyboardState[SDL_SCANCODE_RIGHT] ||
      b(DPAD_RIGHT); 
      break;

    case 'D': return 
      SDL_keyboardState[SDL_SCANCODE_DOWN] ||
      b(DPAD_DOWN); 
      break;

    case 'L': return 
      SDL_keyboardState[SDL_SCANCODE_LEFT] ||
      b(DPAD_LEFT); 
      break;

    case 'E': return SDL_keyboardState[SDL_SCANCODE_ESCAPE]; break;
...
const uint8_t *SDL_keyboardState;
uint8_t SDL_screen[SDL_SCREEN_WIDTH * SDL_SCREEN_HEIGHT];

void SAF_FE_drawPixel(uint8_t x, uint8_t y, uint8_t color)
{
#if SAF_SETTING_SDL2_TINY_SCALE == 1
  SDL_screen[y * SDL_SCREEN_WIDTH + x] = color;
#else
  uint8_t *pixel = SDL_screen +
...
uint8_t SAF_FE_buttonPressed(uint8_t button)
{
  switch (button)
  {
    case SAF_BUTTON_UP: return 
      SDL_keyboardState[SDL_SCANCODE_W] ||
      SDL_keyboardState[SDL_SCANCODE_UP]; 
      break;

    case SAF_BUTTON_DOWN: return 
      SDL_keyboardState[SDL_SCANCODE_S] ||
      SDL_keyboardState[SDL_SCANCODE_DOWN]; 
      break;

    case SAF_BUTTON_LEFT: return 
      SDL_keyboardState[SDL_SCANCODE_A] ||
      SDL_keyboardState[SDL_SCANCODE_LEFT]; 
      break;

    case SAF_BUTTON_RIGHT: return 
      SDL_keyboardState[SDL_SCANCODE_D] ||
      SDL_keyboardState[SDL_SCANCODE_RIGHT]; 
      break;

    case SAF_BUTTON_A: return 
      SDL_keyboardState[SDL_SCANCODE_Y] ||
      SDL_keyboardState[SDL_SCANCODE_Z] ||
      SDL_keyboardState[SDL_SCANCODE_J] ||
      SDL_keyboardState[SDL_SCANCODE_SPACE]; 
      break;

    case SAF_BUTTON_B: return 
      SDL_keyboardState[SDL_SCANCODE_X] ||
      SDL_keyboardState[SDL_SCANCODE_K] ||
      SDL_keyboardState[SDL_SCANCODE_RETURN]; 
      break;

    case SAF_BUTTON_C: return 
      SDL_keyboardState[SDL_SCANCODE_C] ||
      SDL_keyboardState[SDL_SCANCODE_L];
      break;
...
//    SDL_PumpEvents();
    ...
    SDL_UpdateTexture(texture,NULL,SDL_screen,SDL_SCREEN_WIDTH);
...
sfClock *CSFML_clock;
sfRenderWindow* CSFML_window;
sfTexture* CSFML_windowTexture;
sfSprite* CSFML_windowSprite;

uint8_t CSFML_screenSize = SAF_SCREEN_WIDTH;

uint8_t CSFML_SDL_pixelArtUpscale = 0;
uint8_t *CSFML_upscaledScreen = 0;

#if SAF_SETTING_ENABLE_SOUND
  sfSound *CSFML_sound;
  sfSoundBuffer *CSFML_sounds[SAF_SOUNDS];
  uint8_t CSFML_previousVolume = 0;
#endif

uint32_t CSFML_windowPixels[SAF_SCREEN_WIDTH * SAF_SCREEN_HEIGHT * 4];
uint32_t CSFML_paletteRGB32[256]; // SFML can't do 332, so precompute RGB here

void SAF_FE_playSound(uint8_t sound)
{
#if SAF_SETTING_ENABLE_SOUND
  sfSound_setBuffer(CSFML_sound,CSFML_sounds[sound]);
  sfSound_play(CSFML_sound);
#else
  _SAF_UNUSED(sound);
#endif
}
...
void SAF_FE_GF_init(uint8_t CLIParameters[128])
{
  for (int i = 0; i < 256; ++i) // precompute RGB palette
  {
    ...
    CSFML_paletteRGB32[i] = 0xff000000 | (((uint32_t) b) << 16) |
      (((uint32_t) g) << 8) | r;
  }

  if (CLIParameters['u'] != 0)
  {
    CSFML_SDL_pixelArtUpscale = 1;
    CSFML_upscaledScreen = malloc(SAF_SCREEN_WIDTH * SAF_SCREEN_WIDTH * 4);
    CSFML_screenSize *= 2;
  }

  for (int i = 0; i < CSFML_screenSize * CSFML_screenSize; ++i)
    CSFML_windowPixels[i] = 0;
  
  CSFML_clock = sfClock_create();
  sfClock_restart(CSFML_clock);

  uint16_t screenScale = CLIParameters['s'] - '0';

  uint8_t fullscreen = screenScale == 0;

  CSFML_windowTexture = sfTexture_create(CSFML_screenSize,
    CSFML_screenSize);

  sfTexture_setSmooth(CSFML_windowTexture,sfFalse);

  CSFML_windowSprite = sfSprite_create();

  sfVideoMode mode = {CSFML_screenSize, CSFML_screenSize, 32};

  CSFML_window = sfRenderWindow_create(mode, SAF_PROGRAM_NAME, 
  ...
  sfWindow_setPosition(CSFML_window,winPos);

  sfSprite_setTexture(CSFML_windowSprite,CSFML_windowTexture,sfTrue);

  sfWindow_setVerticalSyncEnabled((sfWindow *) CSFML_window,sfFalse);

  if (screenScale != 0)
  {
    sfVector2u winSize;
    winSize.x = CSFML_screenSize * screenScale;
    winSize.y = winSize.x;
    sfWindow_setSize(CSFML_window,winSize);
  }

#if SAF_SETTING_ENABLE_SOUND
  CSFML_sound = sfSound_create();

  {
    ...
    for (int j = 0; j < SAF_SOUNDS; ++j)
    {
      ...
      CSFML_sounds[j] = sfSoundBuffer_createFromSamples(
...
{
#if SAF_SETTING_ENABLE_SOUND
  sfSound_destroy(CSFML_sound);

  for (int i = 0; i < SAF_SOUNDS; ++i)
    sfSoundBuffer_destroy(CSFML_sounds[i]);
#endif

  sfClock_destroy(CSFML_clock);
  sfRenderWindow_destroy(CSFML_window);
  sfSprite_destroy(CSFML_windowSprite);
  sfTexture_destroy(CSFML_windowTexture);

  if (CSFML_SDL_pixelArtUpscale)
    free(CSFML_upscaledScreen);
}
...
uint8_t SAF_FE_GF_loop(uint8_t params[128])
{
  ...
  while (sfRenderWindow_pollEvent(CSFML_window,&event));

  if (!sfRenderWindow_isOpen(CSFML_window))
    return 0;
  
#if SAF_SETTING_ENABLE_SOUND
  int v = params['v'] - '0';

  if (CSFML_previousVolume != v)
  {
    sfSound_setVolume(CSFML_sound,(v * 100) / 8);
    CSFML_previousVolume = v;
  }
#endif
  ...
  const uint8_t screen[SAF_SCREEN_WIDTH * SAF_SCREEN_HEIGHT])
{
  ...
  uint32_t *pixel2 = CSFML_windowPixels;

  if (CSFML_SDL_pixelArtUpscale)
  {
    SAF_FE_scale2xScreen(screen,CSFML_upscaledScreen);
    pixel = CSFML_upscaledScreen;
  }

  for (int i = 0; i < CSFML_screenSize * CSFML_screenSize; ++i)
  {
    *pixel2 = CSFML_paletteRGB32[*pixel];
    pixel++;
    pixel2++;
  }

  sfTexture_updateFromPixels(CSFML_windowTexture,
    (const sfUint8 *) CSFML_windowPixels,
    CSFML_screenSize,CSFML_screenSize,0,0);

  sfRenderWindow_drawSprite(CSFML_window,CSFML_windowSprite,NULL);
  sfRenderWindow_display(CSFML_window);
}
...
uint32_t SAF_FE_GF_sleep(uint16_t sleepMs)
{
  ...
  sfSleep(t);
  return sfClock_getElapsedTime(CSFML_clock).microseconds / 1000;
}
...
uint8_t x11Scr[SAF_SCREEN_WIDTH * SAF_SCREEN_HEIGHT];

uint8_t x11Buttons[SAF_BUTTONS];

uint8_t x11CurrentSound = 0;
uint32_t x11SoundEnd = 0;
...
void SAF_FE_drawPixel(uint8_t x, uint8_t y, uint8_t color)
{
  x11Scr[y * SAF_SCREEN_WIDTH + x] = color;
}

void SAF_FE_playSound(uint8_t sound)
{
  x11CurrentSound = sound;
  x11SoundEnd = x11GetTime() + 1000;
...
int main(int argc, char **argv)
{
  ...
  while (1) // main loop
  {
    ...
    if (x11CurrentSound != previousSound)
    {
      switch (x11CurrentSound)
      {
        case SAF_SOUND_BEEP: 
    ...
    while (XCheckWindowEvent(display,window,KeyPressMask | KeyReleaseMask,&event) != False)
    {
      ...
      switch (XKeycodeToKeysym(display,event.xkey.keycode,0))
      {
        ...
        case XK_Up: case XK_w: 
          x11Buttons[SAF_BUTTON_UP] = state; break;

        case XK_Left: case XK_a:
          x11Buttons[SAF_BUTTON_LEFT] = state; break;
          
        case XK_Right: case XK_d:
          x11Buttons[SAF_BUTTON_RIGHT] = state; break;

        case XK_Down: case XK_s:
          x11Buttons[SAF_BUTTON_DOWN] = state; break;

        case XK_y: case XK_z: case XK_j: case XK_space:
          x11Buttons[SAF_BUTTON_A] = state; break;
        
        case XK_x: case XK_k: case XK_Return:
          x11Buttons[SAF_BUTTON_B] = state; break;

        case XK_c: case XK_l:
          x11Buttons[SAF_BUTTON_C] = state; break;
